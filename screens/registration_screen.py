...
class RegistrationScreen(QWidget):
    ...
    def start_registration(self, person_name, employee_id):
    ...
    def display_camera_feed(self, frame_rgb):
        ...
        try:
            ...
            if label_w > 50:
                ...
                scaled_pixmap = pixmap.scaled(label_w, target_height, Qt.KeepAspectRatio, Qt.SmoothTransformation)
                self.camera_label.setPixmap(scaled_pixmap)
            else:
                self.camera_label.setPixmap(pixmap)
            if not self.camera_label.isVisible(): self.camera_label.setVisible(True)
    ...
    def capture_registration_face(self):
        if self.current_frame is None or self.current_registration_step >= len(self.registration_steps):
            return
        ...
        if face_img is not None:
            ...
            is_valid, message, quality = self.face_recognizer.validate_face_sample(face_img, landmarks, check_liveness=False)
            if is_valid and quality >= 0.7:
                self.captured_faces.append(face_img.copy())
                ...
                if self.current_registration_step >= len(self.registration_steps):
                    ...
                    self.instruction_label.setText(self.registration_steps[self.current_registration_step]["instruction"])
            else:
    ...
    def complete_registration(self):
        ...
        for face_img in self.captured_faces:
            ...
            if is_valid: final_samples.append(face_img)
    ...
    def cancel_registration(self):
        reply = QMessageBox.question(self, "Cancel", "Cancel registration?", QMessageBox.Yes | QMessageBox.No)
